...
- **daemon/** — our fork of alleycat (GPL-3.0), pinned to **iroh 1.0.0** so it
  ...
  *canary* relays). Owns the iroh endpoint, identity, pairing, and the
  codex app-server transport. `pair --url` emits a QR that opens this PWA
  already paired.
...
```
bun run dev        # build daemon + serve PWA on the LAN + ensure daemon + print QR
```

Scan the QR (phone on the same wifi) and the PWA opens connected. The daemon runs
in the background and persists; Ctrl-C only stops serving. `bun run stop` stops
the daemon. (Desktop sanity check: paste the printed URL into a browser.)

Individual pieces, if you want them:

```
bun run daemon                         # just run the forked daemon (owns codex + iroh)
bun run pair                           # print a QR (defaults to the hosted PWA URL)
bun run web                            # just serve web/ on :8123
bun run stop                           # stop the daemon
DOGGYPILE_WEB=http://192.168.1.5:8123 bun run pair   # QR for a local dev PWA
```
...
cargo run -p alleycat --bin alleycat -- pair --qr     # raw alleycat pair payload + QR
cargo run -p alleycat --bin alleycat -- pair --url https://doggypile.app --qr  # PWA-link QR
...
Scan the QR from `pair --url` and the PWA opens connected — sessions, streaming,
send/steer, all over iroh.
