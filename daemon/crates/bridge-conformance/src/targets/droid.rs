...
pub async fn spawn(opts: TargetSpawn) -> Result<TargetHandle> {
    let bridge_bin = opts
        .bridge_bin
        .ok_or_else(|| anyhow!("droid target requires bridge_bin"))?;
    let droid_bin = opts
        .backend_bin
        .ok_or_else(|| anyhow!("droid target requires backend_bin"))?;
    let codex_home = TempDir::new().context("codex home tempdir")?;

    let mut child = Command::new(&bridge_bin)
        .env("DROID_BRIDGE_DROID_BIN", &droid_bin)
    ...
    tee_stream("droid-bridge", stderr);
