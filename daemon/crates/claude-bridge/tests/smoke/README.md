...
running `doggypile` daemon).

| Script | What it does |
|---|---|
| `stdio_smoke.sh` | Spawns `cargo run -p doggypile-claude-bridge` in stdio mode and feeds it a scripted JSON-RPC sequence (`initialize` → `thread/start` → `turn/start "say hi in one word"` → wait for `turn/completed`). Talks to **real claude**. Requires `claude` on `$PATH` and `jq`. |
| `daemon_smoke.sh` | Connects to a running `doggypile` daemon via the iroh client (`doggypile probe`) and runs the same flow against the `claude` agent. Requires `doggypile serve` running in another terminal. |
...
# daemon-mediated:
target/release/doggypile serve     # in another terminal
