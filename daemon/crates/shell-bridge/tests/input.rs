...
#[tokio::test]
async fn input_writes_to_interactive_shell_session() {
    ...
    client
        .request(
            ...
            json!({
                ...
                "data_b64": STANDARD.encode(b"printf 'ready-from-input\\n'; exit 0\n")
            }),
        )
        .await;
    ...
    assert!(
        output.contains("ready-from-input"),
        "expected shell/input output, got {output:?}"
    );
